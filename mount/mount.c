...
static char mount_folder[PATH_MAX];
...
void read_data_block(int block_id, uint8_t *buffer, size_t size) {
    if (block_id < 3 || block_id >= sb.total_blocks) {
        fprintf(stderr, "Intento de leer bloque inválido: %d\n", block_id);
        memset(buffer, 0, size);
        return;
    }
...
int main(int argc, char *argv[]) {
    ...
    char real_fs_folder[PATH_MAX];
    if (!realpath(argv[1], real_fs_folder)) {
        perror("Error resolviendo ruta de carpeta_fs");
        return 1;
    }

    strncpy(mount_folder, real_fs_folder, sizeof(mount_folder));
    cargar_metadatos();
